...
/* 串畫面區 */
  ...
  const emit = defineEmits(['GoHome','GoSensorDoorAutomationList','GoSensorDoorHistoryFull']);
  /* 內部函數定義 : 該畫面按鈕點擊後會觸發的函數 */
  function GoHome(){
    emit('GoHome');
  }
  function GoSensorDoorAutomationList(){
    emit('GoSensorDoorAutomationList');
  }
...
<template>
  <div class="all">
    <div class="標題1">
      <img class="door回上一頁" src="/icon/回上一頁.svg" @click="GoHome">  
      <div>門窗感應器</div>
    </div>
    <div class="區域框">
    ...
    <div class="歷史紀錄區">
      <div class="標題2">
        ...
        <div class="顯示更多資訊字" @click="GoSensorDoorHistoryFull">顯示更多資訊</div>
      </div> 
