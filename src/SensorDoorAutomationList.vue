...
/* 串畫面區 */
  ...
  //去快捷訊息的變數
  const emit = defineEmits(['GoSensorDoorHistory','GoSensorDoorAutomation']);
  /* 內部函數定義 : 該畫面按鈕點擊後會觸發的函數 */
  //去快捷訊息的畫面
  function GoSensorDoorHistory(){
    emit('GoSensorDoorHistory');
  }
  function GoSensorDoorAutomation(){
    emit('GoSensorDoorAutomation');
  }
...
// 定義 reactive 變量
const 自動化指令 = ref([
  { text: '當門窗感應器為開啟時，關閉臥室全部設備', color: '#e2dede' },  // 初始為白色
  { text: '當門窗感應器為關閉時，開啟客廳插座', color: '#e2dede' }   // 初始為白色
]);
...
<template>
  <div class="WorkContainer">
    <div class="content">
      <div class="WorkTop">
          <img class="WorkBack" src="/icon/回上一頁.svg" @click="GoSensorDoorHistory">
      </div>
      <div class="動作modeframe">
        <div class="top">
          <div class="text動作">
            <img class="燈泡icon"  src="/icon/門窗感應器.svg">
            <div>自動化內容</div>
          </div>
          <div class="選取列">
        ...
        <div class="動作塊內容">
          ...
          <div class="新增自動化塊" @click="GoSensorDoorAutomation">
            <div class="新增自動化內容">
      ...
      <div class="complete" @click="GoSensorDoorHistory">完成</div>
    </div>
  </div>
</template>
