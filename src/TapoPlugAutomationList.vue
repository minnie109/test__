...
/* 串畫面區 */
  ...
  //去快捷訊息的變數
  const emit = defineEmits(['GoTapoPlugHistory','GoTapoPlugAutomation']);
  /* 內部函數定義 : 該畫面按鈕點擊後會觸發的函數 */
  //去快捷訊息的畫面
  function GoTapoPlugHistory(){
    emit('GoTapoPlugHistory');
  }
  function GoTapoPlugAutomation(){
    emit('GoTapoPlugAutomation');
  }
...
// 定義 reactive 變量
const 自動化指令 = ref([
  { text: '20分鐘後關閉插座', color: '#e2dede' },  // 初始為白色
  { text: '當濕度 > 80% 時,開啟插座', color: '#e2dede' }   // 初始為白色
]);
...
<template>
  <div class="WorkContainer">
    <div class="content">
      <div class="WorkTop">
          <img class="WorkBack" src="/icon/回上一頁.svg" @click="GoTapoPlugHistory">
      </div>
      <div class="動作modeframe">
        ...
        <div class="動作塊內容">
          ...
          <div class="新增自動化塊" @click="GoTapoPlugAutomation">
            <div class="新增自動化內容">
      ...
      <div class="complete" @click="GoTapoPlugHistory">完成</div>
    </div>
  </div>
</template>
