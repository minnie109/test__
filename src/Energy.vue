...
// 定義 reactive 變量
const 自動化指令 = ref([
  { text: '溫濕度感應器檢測到室內溫度低於設定值,發送通知提醒用戶開窗通風,避免開空調或風扇造成的能耗', color: '#e2dede' },  // 初始為白色
  { text: '日落後光線不足，自動開啟房間的智慧燈泡和燈條，但調整到最低亮度，確保基本照明且不浪費電力', color: '#e2dede' },
  { text: '關閉臥室插頭', color: '#e2dede' }
]);
...
const chartData = ref({
  ...
  datasets: [
    {
      label: '昨日累積總用電量',
      backgroundColor: '#B2D099',
      borderColor: '#B2D099', // 折線的顏色
      fill: false, // 不填充區域
      data: [] // 纵轴数据
    }
  ]
...
const options = {
  responsive: true,
  maintainAspectRatio: true, // 使圖表保持寬高比
  scales: {
    y: {
      beginAtZero: false, // 不從 0 開始
      min: 'auto', // 可以設置自動起點
      ticks: {
        // 可以進一步微調刻度顯示
      }
...
// 創建 refs 來存儲設備資訊和目標設備
const devices = ref([])
const targetDevice = ref(null) // 存放特定 entity_id 的設備資訊
const deviceHistory = ref([]) // 用於存放設備的歷史數據

// 定義一個目標 entity_id
const targetEntityId = "sensor.tuya_power_statistics"
...
const fetchDevices = async () => {
  try {
    ...
    devices.value = dataDevice.message

    // 使用 for 迴圈找到特定的 entity_id
    for (let device of devices.value) {
      if (device.entity_id === targetEntityId) {
        targetDevice.value = device
        // 找到設備後，獲取它的歷史資料
        await fetchDeviceHistory(device.entity_id)
        break
      }
    }
  } catch (error) {
...
const fetchDeviceHistory = async (entityId) => {
  try {
    const url = 'http://163.22.17.116:8122/api/history_data';
    const headers = {
      'Content-Type': 'application/json',
    };
    const body = {
    ...
    const historyData = data.message[0] || []; // 假设数据在 message 中

    // 处理历史数据
    const filteredHistory = [];
    let lastValue = null; // 用于存储上一个有效状态的值

    historyData.forEach((entry) => {
      const currentValue = parseFloat(entry.state);

      // 确保 currentValue 是有效数字
      if (!isNaN(currentValue) && entry.state !== null) {
        const formattedValue = currentValue.toFixed(2); // 保留两位小数的字符串

        // 用保留两位小数的值去与上一个有效值比较
        if (formattedValue !== lastValue) {
          // 将 last_changed 格式化为 "YYYY-MM-DD HH:MM"
          const dateObj = new Date(entry.last_changed);
          const formattedMonthDay = `${String(dateObj.getMonth() + 1).padStart(2, '0')}-${String(dateObj.getDate()).padStart(2, '0')}`; // 获取月份和日期部分
  const formattedTime = dateObj.toISOString().split('T')[1].slice(0, 5); // 获取时分部分
  const formattedDateTime = `${formattedMonthDay} ${formattedTime}`; // 合并为 "MM-DD HH:MM" 格式

          filteredHistory.push({
            last_changed: formattedDateTime,
            state: formattedValue, // 使用保留两位小数的值
          });
          lastValue = formattedValue; // 更新上一个值为保留两位小数的值
        }
      }
    });

    deviceHistory.value = filteredHistory; // 存储过滤后的历史数据

    // 更新图表数据
    updateChartData(filteredHistory);
  } catch (error) {
...
const updateChartData = (history) => {
  ...
  history.forEach(entry => {
    chartData.value.labels.push(entry.last_changed); // 横轴数据
    chartData.value.datasets[0].data.push(entry.state); // 纵轴数据
  });
};


// 使用 onMounted 確保在組件加載時發送請求
onMounted(() => {
  fetchDevices()
})
...
<template>
  <div class="WorkContainer">
    <div class="WorkTop">
      ...
      <div class="工作標題">節能</div>
      <div class="電" v-if="targetDevice && targetDevice.state !== 'none'">
        <div class="電量列">
          <img class="電池" src="/icon/綠色電池.svg">
          <p class="度">{{ targetDevice.state }}度</p>
        </div>
        
        <div class="度">5元/度: {{ 5 * targetDevice.state }} 元</div>
      </div>
      <div v-else>
        <p>未找到指定的設備或狀態為無效</p>
      </div>
