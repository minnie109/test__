...
/* 串畫面區 */
  ...
  const emit = defineEmits(['GoHome','GoTuyaPlugAutomationList','GoTuyaPlugHistoryFull']);
  /* 內部函數定義 : 該畫面按鈕點擊後會觸發的函數 */
  function GoHome(){
    emit('GoHome');
  }
  function GoTuyaPlugAutomationList(){
    emit('GoTuyaPlugAutomationList');
  }
  function GoTuyaPlugHistoryFull(){
    emit('GoTuyaPlugHistoryFull');
  }
const entityId = ref('switch.zhi_hui_cha_zuo_socket_1');  // 存放實體ID
...
<template>
  <div class="all">
    <div class="標題1">
      <img class="tuya回上一頁" src="/icon/回上一頁.svg" @click="GoHome">  
      <div>Tuya插座</div>
    </div>
    <div class="區域框">
    ...
    <div class="自動化區">
        <div class="自動化標題">
            <div>自動化</div>
            <img  class="自動化更多" src="/icon/箭頭更多.svg" @click="GoTuyaPlugAutomationList">
        </div>
        <div class="自動化內容">
          <div 
              class="開關" 
    ...
    <div class="歷史紀錄區">
      <div class="標題2">
        ...
        <div class="顯示更多資訊字" @click="GoTuyaPlugHistoryFull">顯示更多資訊</div>
      </div>
